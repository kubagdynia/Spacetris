...
public partial class Menu
{
    private static Func<object, object, object> ScoresCustomSection()
    {
        return (arg1, _) =>
        {
            ...
            DrawBackgroundRectangle(target, scorePosition);
            DrawHeaders(target, scorePosition);

            var offset = 1;
            foreach (ScoreLine scoreLine in GameSettings.GetScores())
            {
                DrawScoreLine(target, scoreLine, scorePosition, offset);
                offset++;
            }
    ...
    private static void DrawBackgroundRectangle(RenderTarget target, Point2 scorePosition)
    {
    ...
    private static void DrawHeaders(RenderTarget target, Point2 scorePosition)
    {
    ...
    private static void DrawScoreLine(RenderTarget target, ScoreLine scoreLine, Point2 scorePosition, int offset)
    {
